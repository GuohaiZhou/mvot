...
{
    using namespace std;
    ifstream in1; in1.open("simulation_configurations_dim5.txt");
    string str,seedO,MisType;
    stringstream ss,ss2;  ofstream osum,osumDts,oSimuLog;
    osum.open("Summary.txt");  // ofstream otwtlr; otwtlr.open("TwTlr.txt");
    osumDts.open("SummaryDetails.txt");   oSimuLog.open("SimuLog.txt");  double beta1;
    string ManualConfirm;
    int MaxFunEval,MaxIterations; double StopCriteria;
...
// int NumerErrorDatExported=0;
   ...
   // int NewtonConvFail, bfgsConvFail,, TrustRegionConvFail;
    int BobyqaConvFail,TwFail,WtsFail;
    // read simulation setting
    int i,j,NumSimu,Sz,dim,NumSimuCbWts;  double rou;
    for(i=1; i<11; ++i)
    { getline(in1,str);
        if(i==4)  NumSimu = atoi(str.c_str());
        if(i==7)  Sz = atoi(str.c_str());
        if(i==10)  dim = atoi(str.c_str());
    }

    dbcolvec mu(dim),misrates(dim); dbmat Sig(dim,dim);
    ...
    for(i=1; i<44; ++i)
    {
        ...
        case 16:
            getline(in1,str);
            rou=strtod(str.c_str(),NULL);
            break;

        case 31:
            getline(in1,str);
            beta1=strtod(str.c_str(),NULL);
            break;
        ...
        case 28:
            ...
            {
                getline(in1,str,',');
                misrates(j)=strtod(str.c_str(),NULL);
            }
            getline(in1,str);
            misrates(j)=strtod(str.c_str(),NULL);
            break;
 ...
 //double StopCriteriaTrustRegion=StopCriteria*10;
    ...
    //long int MaxFunEvalLong2;
    int QuadProg_MaxIter = 500;

  //double BigPositiveEstimate=200,BigNegativeEstimate=-200;


    Sig=rou;  for(i=0; i<dim; ++i)  Sig(i,i)=1;
    ...
    cout<<"Missing mechanism: "<<MisType<<endl<<endl;
    cout<<"missing rates: "<<trans(misrates)<<endl<<endl;
    cout<<"seed: "<<seedO<<endl<<endl;
    cout<<"Number of simulation replications used in the "\
    <<"calculation of chi-bar-weights (irrelevant when dim=2): \
    \n"<<NumSimuCbWts<<endl<<endl;
    cout<<"beta1: "<<beta1<<"\n";
    ...
    else if(confirmed=="Y" || confirmed=="y")
    {   osum<<"simulation replication times: "<<NumSimu<<endl<<endl;
        osum<<"sample size: "<<Sz<<endl<<endl;
        osum<<"true value of mu: "<<trans(mu)<<endl<<endl;
        osum<<"true value of Sig: \n"<<Sig<<endl<<endl;
        osum<<"missing mechanism: "<<MisType<<endl<<endl;
        osum<<"missing rates: "<<trans(misrates)<<endl<<endl;
        osum<<"Stopping criteria: "<<StopCriteria<<"\n";

        osumDts<<"simulation replication times: "<<NumSimu<<endl<<endl;
        osumDts<<"sample size: "<<Sz<<endl<<endl;
        osumDts<<"true value of mu: "<<trans(mu)<<endl<<endl;
        osumDts<<"true value of Sig: \n"<<Sig<<endl<<endl;
        osumDts<<"missing mechanism: "<<MisType<<endl<<endl;
        osumDts<<"Stopping criteria: "<<StopCriteria<<"\n";

        osumDts<<"Specified missing rates: "<<trans(misrates)<<endl<<endl;

        int OdlSubTwRejCounter=0, OdlBdTwRejCounter=0,\
        OdlSubTlrRejCounter=0,OdlBdTlrRejCounter=0;
        int CCSubTwRejCounter=0, CCBdTwRejCounter=0,\
        CCSubTlrRejCounter=0,CCBdTlrRejCounter=0,  CCSubTwRejCounter_h01=0;
        int OdlSubTwRejCounter_h01=0;

        dlib::rand rndSample, rndCBwts,rndMisInd;
        rndSample.set_seed(seedO);

        double SeedForCBWts=rndSample.get_random_gaussian();
        ss<<SeedForCBWts;
        rndCBwts.set_seed( ss.str() );
     ...
     if(MisType.compare("MCAR")==0   )
     {
        // space allocation
        int dimEta=dim*(dim+1)/2;
        int NumPara=dim+dimEta;

        // used for Tw
        dbcolvec CCmuMLEuc(dim), CCEtaMLEuc(dimEta),\
        OdlmuMLEuc(dim), OdlEtaMLEuc(dimEta);
        dbmat CCLMLEuc(dim,dim),OdlLMLEuc(dim,dim);

        dbcolvec CCthetaUc(NumPara),OdlthetaUc(NumPara);
        CCthetaUc=0; OdlthetaUc=0;
        dbcolvec CClbtw(dim),CCubtw(dim),Odllbtw(dim),Odlubtw(dim);
        CCubtw=BIGNUM;   Odlubtw=BIGNUM;
        dbmat CCSamCov(dim,dim);
        // in CC, muMLEuc=sample mean, MLE of Sig=CCSamCov

        // used for Tlr
        long npt=2*NumPara+1;
        dbcolvec CCmuMLEc(dim), CCEtaMLEc(dimEta);
        dbcolvec OdlmuMLEc(dim), OdlEtaMLEc(dimEta);
        dbmat CCLMLEc(dim,dim),OdlLMLEc(dim,dim);
        dbcolvec ub(NumPara),lb(NumPara);
        // for constrained MLE, upper bd is 0 for estimating mu
        lb=-BIGNUM; ub=BIGNUM; set_subm(ub,range(0,dim-1),range(0,0))=0;

        dbcolvec ubUnc(NumPara),lbUnc(NumPara);
        ubUnc = BIGNUM; lbUnc = -BIGNUM;  // used to compute unconstrained MLE from the BOBYQA algorithm

        dbcolvec CCthetaC(NumPara),OdlthetaC(NumPara);
        CCthetaC=0;  OdlthetaC=0;

        // used for p-value computation
        dbmat ShatDCC(dim,dim),ShatInvDCC(dim,dim),ShatDCC_vh01(dim,dim),ShatInvDCC_vh01(dim,dim);;
        dbmat ShatDOdl(dim,dim),ShatInvDOdl(dim,dim),ShatDOdl_vh01(dim,dim),ShatInvDOdl_vh01(dim,dim);
        dbmat InvInfo(NumPara,NumPara);
        // ShatD=Cov(muhat); ShatInvD=inv(ShatD)
        dbcolvec CbWts_vh0(dim+1),CbWts_vh01(dim+1);
        CbWts_vh0=0;  CbWts_vh01 = 0;// store chi-bar-weights



        dbmat SamCovInit(dim,dim);  SamCovInit=0;
        dbcolvec SamMeanInit(dim);  SamMeanInit=0;
        double CCMinMlkUnderH0, CCMinMlkUnderH0UH1,pval,TwTemp,TlrTemp;
        double OdlMinMlkUnderH0,OdlMinMlkUnderH0UH1;
        double CC_Tw,CC_Tlr,Igno_Tw,Igno_Tlr,pval_CC_Tlr_sub,pval_CC_Tlr_bd;
        double pval_Igno_Tlr_bd,pval_Igno_Tlr_sub,pval_Igno_Tw_bd;
        double pval_Igno_Tw_subvh01,pval_Igno_Tw_subvh0;
        double pval_CC_Tw_subvh01,pval_CC_Tw_subvh0,pval_CC_Tw_bd;

   // dbmat CbWtsSam(NumSimuCbWts,dim);
        int SimuDone=0, SimuSkipped=0;

       clock_t tStartTw, tStartTlr;
       double TwCCSecondsUsed=0,TlrCCSecondsUsed=0,TwIgnorableSecondsUsed=0,TlrIgnorableSecondsUsed=0;
    // double TwNonignorableSecondsUsed=0,TlrNonignorableSecondsUsed=0;

       double TwCCSecondsUsed_ThisIter,TlrCCSecondsUsed_ThisIter;
       double TwIgnorableSecondsUsed_ThisIter,TlrIgnorableSecondsUsed_ThisIter;
     // double TwNonignorableSecondsUsed_ThisIter,TlrNonignorableSecondsUsed_ThisIter;

        oSimuLog<<"CC_Tw,CC_Tlr,Igno_Tw,Igno_Tlr,pval_CC_Tlr_sub,pval_CC_Tlr_bd,\
        pval_Igno_Tlr_bd,pval_Igno_Tlr_sub,pval_Igno_Tw_bd,pval_Igno_Tw_subvh01,pval_Igno_Tw_subvh0,\
        pval_CC_Tw_subvh01,pval_CC_Tw_subvh0,pval_CC_Tw_bd,Seconds_Tw_Igno,Seconds_Tlr_Igno"<<endl;
        //simulation loop
        while(SimuDone<NumSimu)
        {
          if( SimuSkipped>(0.4*NumSimu) )
          {
            ...
            //create missing data
            GenMCARMisInd(rndMisInd,Sz,dim,misrates,MisInd);

            osumDts<<"###################\n"\
            <<"Simulation Run No."<<(SimuDone+1)<<"\n"<<"###################\n\n";

            int NumComCases;
            NAomit(Dat, Sz, dim, MisInd, DatCCtemp,NumComCases);
            dbmat DatCCsam(NumComCases,dim);
            osumDts<<"Sample size after CC:\n"<<NumComCases<<"\n\n";
            if( (NumComCases*5) <= NumPara )
            { osumDts<<"Sample size no greater than #parameters, skip simulation instance:\n\n";
             SimuSkipped++;   continue;
            }
            DatCCsam=subm( DatCCtemp,range(0,NumComCases-1),range(0,dim-1) );

          // initial values are CCed sample mean/cov
            SamMeanCov(DatCCsam,NumComCases,dim, SamMeanInit, SamCovInit);

            CCLMLEuc = chol(SamCovInit); LToEta(CCLMLEuc,dim,CCEtaMLEuc);
            set_subm(CCthetaUc,range(0,dim-1),range(0,0) )=SamMeanInit;
            set_subm(CCthetaUc,range(dim,dim+dimEta-1),range(0,0) )=CCEtaMLEuc;

            CCthetaC = CCthetaUc;
            // for bd-constraint MLE: if initial value do not satisfy constraints, adjust to satisfy constraints
            for(i=0; i<dim; ++i)   if(CCthetaC(i) > 0 ) CCthetaC(i)=0;


           OdlthetaC=CCthetaC; OdlthetaUc=CCthetaUc;
          // initial values for the observed data likelihood method are the same those for CC method


            // ###############
            // complete-case  method
            // unconstrained MLE/Cov are sample mean/cov
            //#################
            osumDts<<"-----------Results from complete-case \n\n";
            // unconstrained MLE
            // make sure pDat points to the CCed dataset before computing MLE
            tStartTw = clock();
            pDat=&DatCCsam;
            CCMinMlkUnderH0UH1=MLkOSamCom(CCthetaUc);
           if (!is_finite(CCMinMlkUnderH0UH1) )
            {  osumDts<<"Error in computing MLE, simu instance skipped\n\n";
               continue; }

            CCmuMLEuc = SamMeanInit;
            TwCCSecondsUsed_ThisIter = (double)(clock() - tStartTw)/CLOCKS_PER_SEC;
            osumDts<<"Maximized log-likelihood under H0UH1: "<<(-1.0*CCMinMlkUnderH0UH1)<<"\n";
            osumDts<<"CC, unconstrained MLE of mean (i.e sample mean) \n"<<trans(CCmuMLEuc)<<endl;
            osumDts<<"CC, unconstrained MLE of Cov (i.e sample cov)\n"\
            <<SamCovInit<<endl;


   if( dlib::max(CCmuMLEuc) < 1e-3 )
   {
      osumDts<<"Since the unconstrained MLE already satisfies constraints (up to a numerical tolerance of 1e-3), it is also the constrained MLE.  The p-values associated with both Tw and Tlr are 1.   \n\n";
      SimuDone = SimuDone + 1;  CC_Tw = 0;
      CC_Tlr = 0; pval_CC_Tlr_bd = 1; pval_CC_Tw_bd = 1;

      ShatDCC = subm (inv( HESScdif(MLkOSamCom, CCthetaUc) ),range(0,dim-1),range(0,dim-1));
      ShatInvDCC = inv(ShatDCC);   WtsFail = 0;
      ChiBarWtsDimGt2ch2_BFGS(ShatDCC,ShatInvDCC,dim, NumSimuCbWts,rndCBwts, CbWts_vh01,\
                              WtsFail,QuadProg_MaxIter);
      if( WtsFail == 1) osumDts<<"Chi-bar-weights calculation fails, skip simulation instance\n";
      pval_CC_Tw_subvh0 = ChiBarPvalue(dim,CbWts_vh01,CC_Tw);
      pval_CC_Tw_subvh01 = pval_CC_Tw_subvh0;
      pval_CC_Tlr_sub = pval_CC_Tw_subvh0;

      TlrCCSecondsUsed_ThisIter = TwCCSecondsUsed_ThisIter;
      osumDts<<"Seconds used in the computation of Tw: "<<TwCCSecondsUsed_ThisIter<<"\n";
      osumDts<<"Seconds used in the computation of Tlr: "<<TlrCCSecondsUsed_ThisIter<<"\n";
      osumDts<<"Tlr requires how much additional time (in \%): "\
      <<( (TlrCCSecondsUsed_ThisIter-TwCCSecondsUsed_ThisIter)/TwCCSecondsUsed_ThisIter)*100<<"% \n\n";
       TwCCSecondsUsed  = TwCCSecondsUsed + TwCCSecondsUsed_ThisIter;
       TlrCCSecondsUsed = TlrCCSecondsUsed + TlrCCSecondsUsed_ThisIter;
    ...
    // continue;
            ...
            tStartTlr = clock();
            MLkOSamComCtr=0; pDat=&DatCCsam; BobyqaConvFail=0;
            CCMinMlkUnderH0= find_min_bobyqaConvgFail(MLkOSamCom,
            CCthetaC, npt,    // number of interpolation points
            ...
            TlrCCSecondsUsed_ThisIter = TwCCSecondsUsed_ThisIter + (double)(clock() - tStartTlr)/CLOCKS_PER_SEC;
            // Tlr requires both the constrained MLE and the unconstrained MLE

    if (!is_finite(CCMinMlkUnderH0) || BobyqaConvFail ==1 )
        {  osumDts<<"simulation instances skipped due to non-convergence in the computation of constrained MLE in the CC method. \n"; SimuSkipped++; continue; }

   osumDts<<"Maximized log-likelihood under H0: "<<(-1.0*CCMinMlkUnderH0)<<"\n";

   if(CCMinMlkUnderH0< CCMinMlkUnderH0UH1)  osumDts<<"!warning: maximized log-like under H0 is unexpectedly greater than that under H0UH1.\n";

  // osumDts<<"Since the maximized log-likelihood under H0 is the same as that under H0UH1 (up to a numerical tolerance of 1e-2), the p-values associated with both Tw and Tlr are 1. H0 is not rejected.  \n\n";
    //  SimuDone=SimuDone+1;  TwTemp=0; TlrTemp=0;
   //  otwtlr<<TwTemp<<","<<TlrTemp<<",";

 //  osumDts<<"Seconds used in the computation of Tw: "<<TwCCSecondsUsed_ThisIter<<"\n";
   //   osumDts<<"Seconds used in the computation of Tlr: "<<TlrCCSecondsUsed_ThisIter<<"\n";
   //   osumDts<<"Tlr requires how much additional time (in \%): "<<( (TlrCCSecondsUsed_ThisIter-TwCCSecondsUsed_ThisIter)/TwCCSecondsUsed_ThisIter)*100<<"% \n\n";
    //   TwCCSecondsUsed  = TwCCSecondsUsed + TwCCSecondsUsed_ThisIter;
   //    TlrCCSecondsUsed = TlrCCSecondsUsed + TlrCCSecondsUsed_ThisIter;


            CCmuMLEc=subm(CCthetaC,range(0,dim-1),range(0,0) );
            CCEtaMLEc=subm(CCthetaC,range(dim,NumPara-1),range(0,0) );
            EtaToL(CCEtaMLEc,dim,CCLMLEc);

            osumDts<<"CC, bd-constrained MLE of mean\n"<<trans(CCmuMLEc)<<endl;
            osumDts<<"CC, bd-constrained MLE of Cov\n"\
            <<CCLMLEc*trans(CCLMLEc)<<endl;
            osumDts<<"\n#(loglike evaluation during numerical optimization): "\
            <<MLkOSamComCtr<<endl<<endl;

 /*
   if( ShatDCC(0,0)<0 || ShatDCC(1,1)<0 || ( det(ShatDCC) ) < 0 )
	     {
	      InvInfo=inv( HESScdif(MLkOSamCom, CCthetaUc) ); // if observed information matrix is not positive definite, plug-in OdlthetaUc instead
	      ShatDCC=subm(InvInfo,range(0,dim-1),range(0,dim-1) );
        if( ShatDCC(0,0)<0 || ShatDCC(1,1)<0 || ( det(ShatDCC) ) <0 )
           {
           osumDts<<"In CC case, observed information matrix is not positive definite, simulation instance skipped.\n";
          std::cout<<"In CC case, observed information matrix is not positive definite, simulation instance skipped.\n";  ++SimuSkipped;  continue;
           }
	     }
	     */

            tStartTw = clock();
            ShatDCC_vh01 = subm (inv( HESScdif(MLkOSamCom, CCthetaUc) ),range(0,dim-1),range(0,dim-1));
            ShatInvDCC_vh01 = inv(ShatDCC_vh01);

            // Tw= trans(muhat-proj)*inv(Shat)*(muhat-proj)
            // proj =argmin (trans(muhat-x)*inv(Shat)*(muhat-x)) over x\in O_minus
            // set  th=muhat-x; min QPObj(th) over th>=muhat;
            //  solution is th_sol, then Tw=QPObj(th_sol);
            CClbtw = CCmuMLEuc; pMQP = &ShatInvDCC_vh01; CCubtw = BIGNUM;
            TwFail = 0;
            TwTemp = find_min_box_constrainedConvFail(bfgs_search_strategy(),\
            objective_delta_stop_strategy(1e-5), \
            QPObj, QPObjGrad, CCmuMLEuc, CClbtw,CCubtw,TwFail,QuadProg_MaxIter);
            if(TwFail == 1) { osumDts<<" calculation of Tw fails, skip this simu instance\n";
             SimuSkipped++; continue; }
           osumDts<<"CCmuMLEuc in TW: "<<trans(CCmuMLEuc)<<endl;
            //CCTwRp(k)=TwTemp;
            TwCCSecondsUsed_ThisIter = TwCCSecondsUsed_ThisIter + (double)(clock() - tStartTw)/CLOCKS_PER_SEC;
            CC_Tw = TwTemp;

         ShatDCC = subm(inv( HESScdif(MLkOSamCom, CCthetaC) ),range(0,dim-1),range(0,dim-1) );
         ShatInvDCC = inv(ShatDCC);
         osumDts<<"Cov(Muhat), using Hat_I_u: \n"<<ShatDCC_vh01<<"\n";
         osumDts<<"Cov(Muhat), using Hat_I_c: \n"<<ShatDCC<<"\n";
         osumDts<<"Cov(Muhat), Sig/SamSz: \n"<<Sig/Sz<<"\n";

            osumDts<<"Tw: "<<CC_Tw<<endl;

            tStartTlr = clock();
            TlrTemp=2*(CCMinMlkUnderH0-CCMinMlkUnderH0UH1);
            CC_Tlr = TlrTemp;
            TlrCCSecondsUsed_ThisIter = TlrCCSecondsUsed_ThisIter + (double)(clock() - tStartTlr)/CLOCKS_PER_SEC;
            // CCTlrRp(k)=TlrTemp;

            osumDts<<"Tlr: "<<CC_Tlr<<"\n\n";

            // compute p-vale (or upper bound of p-value)

            // upper bound of p-value
            // upper bound is ( Pr(Xi_{dim-1}^2>=c)+ Pr(Xi_{dim}^2>=c) )/2
            // pchisq(x, double(dim-1) ) returns Pr(Xi_{dim-1}^2<=x)
            pval_CC_Tw_bd = 1-(scythe::pchisq(CC_Tw, double(dim-1))  \
            + scythe::pchisq(CC_Tw, double(dim)) )/2;

            osumDts<<"CC bound method, upper bound of p-value"\
            <<" using Tw: "<<pval_CC_Tw_bd<<"\n\n";
            if(pval_CC_Tw_bd<SigLevel) CCBdTwRejCounter=CCBdTwRejCounter+1;

            pval_CC_Tlr_bd = 1-(scythe::pchisq(CC_Tlr, double(dim-1))  \
            + scythe::pchisq(pval_CC_Tlr_bd, double(dim)) )/2;
            osumDts<<"CC bound method, upper bound of p-value"\
            <<" using Tlr: "<<pval_CC_Tlr_bd<<"\n\n";
            if(pval_CC_Tlr_bd<SigLevel) CCBdTlrRejCounter=CCBdTlrRejCounter+1;

      WtsFail = 0;
      ChiBarWtsDimGt2ch2_BFGS(ShatDCC,ShatInvDCC,dim, NumSimuCbWts,rndCBwts, CbWts_vh01,
                              WtsFail,QuadProg_MaxIter);
      if(WtsFail == 1) { osumDts<<"Chi-bar-weights calculation fails, skip; "; SimuSkipped++; continue;}
      pval_CC_Tw_subvh01 = ChiBarPvalue(dim,CbWts_vh01,CC_Tw);
       if(pval_CC_Tw_subvh01<SigLevel)  CCSubTwRejCounter_h01=CCSubTwRejCounter_h01+1;
            osumDts<<"Using hat_I_u, CC substitution method, p-value based on Tw: "\
            <<pval_CC_Tw_subvh01<<"\n\n";
            // pval substitution method  use MLE under H_0 in the substitution method for better type I error control

         WtsFail = 0;
         ChiBarWtsDimGt2ch2_BFGS(ShatDCC,ShatInvDCC,dim, NumSimuCbWts,rndCBwts, CbWts_vh0,\
                                 WtsFail,QuadProg_MaxIter);
         if(WtsFail == 1) { osumDts<<"Chi-bar-weights calculation fails, skip; "; SimuSkipped++; continue;}

         pval_CC_Tw_subvh0 = ChiBarPvalue(dim,CbWts_vh0,CC_Tw);
         if(pval_CC_Tw_subvh0<SigLevel)  CCSubTwRejCounter=CCSubTwRejCounter+1;
            osumDts<<"Using hat_I_c, CC substitution method, p-value based on Tw: "\
            <<pval_CC_Tw_subvh0<<"\n\n";

            pval_CC_Tlr_sub = ChiBarPvalue(dim,CbWts_vh0,CC_Tlr);
            if(pval_CC_Tlr_sub<SigLevel)  CCSubTlrRejCounter=CCSubTlrRejCounter+1;

            osumDts<<"Using hat_I_c, CC substitution method, p-value based on Tlr: "<<pval<<"\n\n";
      ...
      osumDts<<"Tlr requires how much additional time (in \%): "<<( (TlrCCSecondsUsed_ThisIter-TwCCSecondsUsed_ThisIter)/TwCCSecondsUsed_ThisIter)*100<<"% \n\n";
       ...
       TlrCCSecondsUsed = TlrCCSecondsUsed + TlrCCSecondsUsed_ThisIter;
            ...
            // ########################
            // unconstrained MLE
            osumDts<<"-----------Results from observed-data likelihood(Odl) \n\n";

            tStartTw = clock();
            MLkOSamMARCtr=0; pDat=&Dat; PMisInd=&MisInd;
          // NewtonConvFail=0;
      //      OdlMinMlkUnderH0UH1
      ...
      //      -BIGNUM,NewtonConvFail,MaxIterations);
            ...
            BobyqaConvFail=0;
            OdlMinMlkUnderH0UH1 = find_min_bobyqaConvgFail(MLkOSamMAR,
            OdlthetaUc,npt,    // number of interpolation points
            lbUnc,  // lower bound constraint
            ubUnc,   // upper bound constraint
            10,    // initial trust region radius
            StopCriteria,  // stopping trust region radius
            MaxFunEvalLong, // max number of objective function evaluations
            BobyqaConvFail );
            ...
            TwIgnorableSecondsUsed_ThisIter = (double)(clock() - tStartTw)/CLOCKS_PER_SEC;
 // cout<<"seconds used for Unconstrained MLE: "<<TwIgnorableSecondsUsed_ThisIter<<endl;

    if (!is_finite(OdlMinMlkUnderH0UH1) ) {  osumDts<<"Error in computing unconstrained MLE, simu instance skipped\n\n";  SimuSkipped = SimuSkipped + 1;
     continue; }


 if(BobyqaConvFail==1)  { osumDts<<"simulation instances skipped due to non-convergence in the computation of unconstrained MLE based on the observed data likelihood using newton method.\n"; SimuSkipped=SimuSkipped+1; continue;}

      OdlmuMLEuc=subm(OdlthetaUc,range(0,dim-1),range(0,0) );

   if( dlib::max(OdlthetaUc) < 1e-3  )
   {
       osumDts<<"unconstrained MLE mu hat: "<<trans( OdlmuMLEuc )<<"\n";
       osumDts<<"Since the unconstrained MLE already satisfies constraints (up to a numerical tolerance of 1e-3), it is also the constrained MLE.  The p-values associated with both Tw and Tlr are 1.   \n\n";
       SimuDone=SimuDone+1;  Igno_Tw=0; Igno_Tlr=0;
       pval_Igno_Tlr_bd = 1; pval_Igno_Tw_bd = 1;

       ShatDOdl_vh01 = subm(inv( HESScdif(MLkOSamMAR, OdlthetaUc) ),range(0,dim-1),range(0,dim-1) );
       ShatInvDOdl_vh01 = inv(ShatDOdl_vh01);
       WtsFail = 0;
       ChiBarWtsDimGt2ch2_BFGS(ShatDOdl_vh01,ShatInvDOdl_vh01,dim,  NumSimuCbWts,rndCBwts,\
        CbWts_vh01,WtsFail,QuadProg_MaxIter);
       if(WtsFail == 1) { osumDts<<"Chi-bar-weights calculation fails, skip; "; SimuSkipped++; continue;}


       pval_Igno_Tw_subvh01 = ChiBarPvalue(dim,CbWts_vh01,Igno_Tw);
       pval_Igno_Tw_subvh0 = pval_Igno_Tw_subvh01;  pval_Igno_Tlr_sub = pval_Igno_Tw_subvh01;

      TlrIgnorableSecondsUsed_ThisIter =  TwIgnorableSecondsUsed_ThisIter;
      osumDts<<"Seconds used in the computation of Tw: "<<TwIgnorableSecondsUsed_ThisIter<<"\n";
      osumDts<<"Seconds used in the computation of Tlr: "<<TlrIgnorableSecondsUsed_ThisIter<<"\n";
      osumDts<<"Tlr requires how much additional time (in \%): "<<( (TlrIgnorableSecondsUsed_ThisIter-TwIgnorableSecondsUsed_ThisIter)/TwIgnorableSecondsUsed_ThisIter)*100<<"% \n\n";
       TwIgnorableSecondsUsed  = TwIgnorableSecondsUsed + TwIgnorableSecondsUsed_ThisIter;
       TlrIgnorableSecondsUsed = TlrIgnorableSecondsUsed + TlrIgnorableSecondsUsed_ThisIter;

   }


        else {
         OdlEtaMLEuc=subm(OdlthetaUc,range(dim,NumPara-1),range(0,0) );

            EtaToL(OdlEtaMLEuc,dim,OdlLMLEuc);
            osumDts<<"Maximized log-likelihood under H0UH1: "<<(-1.0*OdlMinMlkUnderH0UH1)<<"\n";
            osumDts<<"Odl, unconstrained MLE of mean\n"<<trans(OdlmuMLEuc)<<endl;
            osumDts<<"Odl, unconstrained MLE of Cov\n"\
            <<OdlLMLEuc*trans(OdlLMLEuc)<<endl;
            osumDts<<"\n#(loglike evaluation during numerical optimization): "\
            <<MLkOSamMARCtr<<endl<<endl;
            ...
            MLkOSamMARCtr=0; pDat=&Dat; PMisInd=&MisInd;
            BobyqaConvFail=0;
            OdlMinMlkUnderH0 = find_min_bobyqaConvgFail(MLkOSamMAR,
            OdlthetaC,npt,    // number of interpolation points
            lb,  // lower bound constraint
            ub,   // upper bound constraint
            10,    // initial trust region radius
            StopCriteria,  // stopping trust region radius
            ...
            TlrIgnorableSecondsUsed_ThisIter = TwIgnorableSecondsUsed_ThisIter + (double)(clock() - tStartTlr)/CLOCKS_PER_SEC;
 // cout<<"seconds used for unconstrained + constrained MLE: "<<TlrIgnorableSecondsUsed_ThisIter<<endl;

    if (!is_finite(OdlMinMlkUnderH0) ) {  osumDts<<"Error in computing constrained MLE, simu instance skipped\n\n"; SimuSkipped++; continue; }



    if(BobyqaConvFail==1){ osumDts<<"simulation instances skipped due to non-convergence in the computation of constrained MLE based on the observed data likelihood using bobyqa.\n"; SimuSkipped++; continue;}

    osumDts<<"Maximized log-likelihood under H0: "<<(-1.0*OdlMinMlkUnderH0)<<"\n";


      if(OdlMinMlkUnderH0< OdlMinMlkUnderH0UH1)  osumDts<<"!warning: maximized log-like under H0 is unexpectedly greater than that under H0UH1.\n";
  //    osumDts<<"Since the maximized log-likelihood under H0 is the same as that under H0UH1 (up to a numerical tolerance of 1e-2), the p-values associated with both Tw and Tlr are 1.   \n\n";
   //   SimuDone=SimuDone+1;  TwTemp=0; TlrTemp=0;
   // otwtlr<<TwTemp<<","<<TlrTemp<<"\n";
   ...
   //   osumDts<<"Seconds used in the computation of Tw: "<<TwIgnorableSecondsUsed_ThisIter<<"\n";
    ...
    //  osumDts<<"Tlr requires how much additional time (in \%): "<<( (TlrIgnorableSecondsUsed_ThisIter-TwIgnorableSecondsUsed_ThisIter)/TwIgnorableSecondsUsed_ThisIter)*100<<"% \n\n";
       ...
       TlrIgnorableSecondsUsed = TlrIgnorableSecondsUsed + TlrIgnorableSecondsUsed_ThisIter;
            ...
            OdlmuMLEc=subm(OdlthetaC,range(0,dim-1),range(0,0) );
            OdlEtaMLEc=subm(OdlthetaC,range(dim,NumPara-1),range(0,0) );
            EtaToL(OdlEtaMLEc,dim,OdlLMLEc);

            osumDts<<"Odl, bd-constrained MLE of mean\n"<<trans(OdlmuMLEc)<<endl;
            osumDts<<"Odl, bd-constrained MLE of Cov\n"\
            <<OdlLMLEc*trans(OdlLMLEc)<<endl;
            osumDts<<"\n#(loglike evaluation during numerical optimization): "\
            <<MLkOSamMARCtr<<endl<<endl;
         ...
         ShatDOdl_vh01=subm(inv( HESScdif(MLkOSamMAR, OdlthetaUc) ),range(0,dim-1),range(0,dim-1) );
          ShatInvDOdl_vh01 = inv(ShatDOdl_vh01);
         WtsFail = 0;
         ChiBarWtsDimGt2ch2_BFGS(ShatDOdl_vh01,ShatInvDOdl_vh01,dim,  NumSimuCbWts,\
                                 rndCBwts, CbWts_vh01, WtsFail,QuadProg_MaxIter);
         if(WtsFail == 1) { osumDts<<"Chi-bar-weights calculation fails, skip; "; SimuSkipped++; continue;}


            // use MLE under H_0 in the substitution method for better type I error control
                  // ShatD is Cov(muHat)
          ShatDOdl=subm(inv( HESScdif(MLkOSamMAR, OdlthetaC) ),range(0,dim-1),range(0,dim-1) );
          ShatInvDOdl = inv(ShatDOdl);
          WtsFail = 0;
         ChiBarWtsDimGt2ch2_BFGS(ShatDOdl,ShatInvDOdl,dim,  NumSimuCbWts,rndCBwts, CbWts_vh0,\
                            WtsFail,QuadProg_MaxIter);
         if(WtsFail == 1) { osumDts<<"Chi-bar-weights calculation fails, skip; "; SimuSkipped++; continue;}


    /*
     if( ShatDOdl(0,0)<0 || ShatDOdl(1,1)<0 || ( det(ShatDOdl) ) < 0 )
	     {
	      InvInfo=inv( HESScdif(MLkOSamMAR, OdlthetaUc) ); // if observed information matrix is not positive definite, plug-in OdlthetaUc instead
          ShatDOdl=subm(InvInfo,range(0,dim-1),range(0,dim-1) );


    if( ShatDOdl(0,0)<0 || ShatDOdl(1,1)<0 || ( det(ShatDOdl) ) <0 )
           {
          osumDts<<"In Odl case, observed information matrix is not positive definite, simulation instance skipped.\n";
          std::cout<<"In Odl case, observed information matrix is not positive definite, simulation instance skipped.\n";  ++SimuSkipped; continue;
           }
	     }
     */
         osumDts<<"Cov(muhat), when muhat=sample mean from a full sample:\n"<<Sig/Sz<<endl;
         osumDts<<"CC, estimated Cov(muhat):\n"<<ShatDCC<<endl;

        osumDts<<"Odl estimated Cov(muhat):\n"<<ShatDOdl<<endl;
            ...
            //  solution is th_sol, then Tw=QPObj(th_sol);
             tStartTw = clock();
            Odllbtw = OdlmuMLEuc; pMQP=&ShatInvDOdl_vh01;
            TwFail = 0;
            TwTemp = find_min_box_constrainedConvFail(bfgs_search_strategy(),\
            objective_delta_stop_strategy(StopCriteria), \
            QPObj, QPObjGrad, OdlmuMLEuc, Odllbtw,Odlubtw,TwFail,QuadProg_MaxIter);
            if(TwFail == 1) { osumDts<<" calculation of Tw fails, skip this simu instance\n";
             SimuSkipped++; continue; }
            Igno_Tw = TwTemp;
 //cout<<"seconds used for step 2 of Tw: "<<( double( clock() - tStartTw)/CLOCKS_PER_SEC )<<endl;
            TwIgnorableSecondsUsed_ThisIter =  TwIgnorableSecondsUsed_ThisIter \
            + double( clock() - tStartTw)/CLOCKS_PER_SEC;
            // OdlTwRp(k)=TwTemp;
     osumDts<<"Tw: "<<Igno_Tw<<endl;

            tStartTlr = clock();
            TlrTemp = 2*(OdlMinMlkUnderH0-OdlMinMlkUnderH0UH1);
            Igno_Tlr = TlrTemp;
 //cout<<"seconds used for final calculation of Tlr "<<( double( clock() - tStartTlr)/CLOCKS_PER_SEC )<<endl;

            TlrIgnorableSecondsUsed_ThisIter =  TlrIgnorableSecondsUsed_ThisIter \
            + double( clock() - tStartTlr)/CLOCKS_PER_SEC;
            // OdlTlrRp(k)=TlrTemp;

        osumDts<<"Tlr: "<<Igno_Tlr<<"\n\n";
    // otwtlr<<TwTemp<<","<<TlrTemp<<"\n";
            ...
            // upper bound is ( Pr(Xi_{dim-1}^2>=c)+ Pr(Xi_{dim}^2>=c) )/2
            // pchisq(x, double(dim-1) ) returns Pr(Xi_{dim-1}^2<=x)
            pval_Igno_Tw_bd = 1-(scythe::pchisq(Igno_Tw, double(dim-1))  \
            + scythe::pchisq(Igno_Tw, double(dim)) )/2;
            osumDts<<"Odl bound method, upper bound of p-value"\
            <<" using Tw: "<<pval_Igno_Tw_bd<<"\n\n";
            if(pval_Igno_Tw_bd<SigLevel) OdlBdTwRejCounter=OdlBdTwRejCounter+1;

            pval_Igno_Tlr_bd = 1-(scythe::pchisq(Igno_Tlr, double(dim-1))  \
            + scythe::pchisq(Igno_Tlr, double(dim)) )/2;
            osumDts<<"Odl bound method, upper bound of p-value"\
            <<" using Tlr: "<<pval_Igno_Tlr_bd<<"\n\n";
            if(pval_Igno_Tlr_bd<SigLevel) OdlBdTlrRejCounter=OdlBdTlrRejCounter+1;

            //substitution p-value
            pval_Igno_Tw_subvh01 = ChiBarPvalue(dim,CbWts_vh01,Igno_Tw);
            if(pval_Igno_Tw_subvh01<SigLevel)  OdlSubTwRejCounter_h01++;
            osumDts<<"Using hatI_u, Odl substitution method, p-value based on Tw: "\
            <<pval_Igno_Tw_subvh01<<"\n\n";

            pval_Igno_Tw_subvh0 = ChiBarPvalue(dim,CbWts_vh0,Igno_Tw);
            if(pval_Igno_Tw_subvh0<SigLevel)  OdlSubTwRejCounter++;
            osumDts<<"Using hatI_c, Odl substitution method, p-value based on Tw: "\
            <<pval_Igno_Tw_subvh0<<"\n\n";

            pval_Igno_Tlr_sub = ChiBarPvalue(dim,CbWts_vh0,Igno_Tlr);
            if(pval_Igno_Tlr_sub<SigLevel)  OdlSubTlrRejCounter++;
            osumDts<<"Using hatI_c, Odl substitution method, p-value based on Tw: "\
            <<pval_Igno_Tlr_sub<<"\n\n";

     // otwtlr<<TwTemp<<","<<TwTemp<<endl;
      osumDts<<"Seconds used in the computation of Tw: "<<TwIgnorableSecondsUsed_ThisIter<<"\n";
      osumDts<<"Seconds used in the computation of Tlr: "<<TlrIgnorableSecondsUsed_ThisIter<<"\n";
      osumDts<<"Tlr requires how much additional time (in \%): "<<( (TlrIgnorableSecondsUsed_ThisIter - TwIgnorableSecondsUsed_ThisIter)/TwIgnorableSecondsUsed_ThisIter)*100<<"% \n\n";
        }

     oSimuLog<<CC_Tw<<","<<CC_Tlr<<","<<Igno_Tw<<","\
     <<Igno_Tlr<<","<<pval_CC_Tlr_sub<<","<<pval_CC_Tlr_bd<<","\
     <<pval_Igno_Tlr_bd<<","<<pval_Igno_Tlr_sub<<","<<\
     pval_Igno_Tw_bd<<","<<pval_Igno_Tw_subvh01<<","<<pval_Igno_Tw_subvh0\
     <<","<<pval_CC_Tw_subvh01<<","<<pval_CC_Tw_subvh0<<","\
     <<pval_CC_Tw_bd<<","<<TwIgnorableSecondsUsed_ThisIter<<","<<TlrIgnorableSecondsUsed_ThisIter<<"\n";

     SimuDone = SimuDone+1;
        }
        osum<<"In % \n";
       // osum<<"CCBdTwRejProp,CCBdTlrRejProp,CCSubTwRejProp,CCSubTlrRejProp";
      //  osum<<",OdlBdTwRejProp,OdlBdTlrRej Prop,OdlSubTwRejProp,OdlSubTlrRejProp \n";
 osum<<"Substitution method (T_W (vh01), T_LR, T_CC) && Bound method (T_W, T_LR, T_CC) \n";

        double CCBdTwRejProp,  CCBdTlrRejProp, \
        CCSubTwRejProp,  CCSubTlrRejProp,CCSubTwRejProp_h01;

        CCBdTwRejProp = CCBdTwRejCounter/double(NumSimu);
        CCBdTlrRejProp = CCBdTlrRejCounter/double(NumSimu);

        CCSubTwRejProp = CCSubTwRejCounter/double(NumSimu);
        CCSubTwRejProp_h01 = CCSubTwRejCounter_h01/double(NumSimu);
        CCSubTlrRejProp = CCSubTlrRejCounter/double(NumSimu);


        double OdlBdTwRejProp,  OdlBdTlrRejProp, \
        OdlSubTwRejProp,  OdlSubTlrRejProp,OdlSubTwRejProp_h01;

        OdlBdTwRejProp = OdlBdTwRejCounter/double(NumSimu);
        OdlBdTlrRejProp = OdlBdTlrRejCounter/double(NumSimu);

        OdlSubTwRejProp = OdlSubTwRejCounter/double(NumSimu);
        OdlSubTlrRejProp = OdlSubTlrRejCounter/double(NumSimu);
        OdlSubTwRejProp_h01 = OdlSubTwRejCounter_h01/double(NumSimu);

        osum<<setprecision (2) << fixed<<"& "<<(OdlSubTwRejProp_h01*100)<<"& "<<(OdlSubTlrRejProp*100)<<"& "<<(CCSubTlrRejProp*100) \
<<"&& "<<(OdlBdTwRejProp*100)<<"& "<<(OdlBdTlrRejProp*100)<<"& "<<(CCBdTwRejProp*100)<<"\n\n";

        osum<<"Reference (not used in manuscript table): bound CC_T_LR & substitution CC_T_w_h01 \
        &  substitution CC_T_w_h0 &  OdlSubTwRejProp (vh0) \n";
        osum<<(CCBdTlrRejProp*100)<<"& "<<(CCSubTwRejProp_h01*100)<<(CCSubTwRejProp*100)\
        <<(OdlSubTwRejProp*100)<<"\n\n";

        double TwAvg = (TwIgnorableSecondsUsed/NumSimu),TlrAvg=(TlrIgnorableSecondsUsed/NumSimu);
        osum<<"On average and under ignorable missingness, seconds used for the calculation of Tw: "<<TwAvg<<"\n";
        osum<<"On average and under ignorable missingness, seconds used for the calculation of Tlr: "<<TlrAvg<<"\n";
        osum<<"On average and under ignorable missingness, Tlr requires how much additional time (in \%): "<<( (TlrAvg - TwAvg)/TwAvg)*100<<"% \n";
        in1.close();
        osum.close();
        oSimuLog.close();
        osumDts.close();
       ...
       {
       cout<<"p>2 is only supports MCAR\n"<<endl;
       return 0;
       }
    }


    else { cout<<"wrong choice (must be one of Y, y, N or n) :\n";
    return 0; }
...
const double& alpha1,double& LogExpectedProbMisXij)
{
    ...
    - alpha1* ( std::sqrt(2) *sig* (*pQHPts)(s,0) +mu  )  ) );
    }
   ...
   for(int j1=0;j1<mQHpts;++j1)
    {
        ...
        wj2=(*pQHPts)(j2,1);
        qj=mu+L*zj;
        out=out + (wj1*wj2)/ (  ( 1+std::exp( -alpha01-alpha11*qj(0) )  ) \
        *( 1+std::exp( -alpha02-alpha12*qj(1) )  )   );
        }
    }
    out=std::log(out)-std::log(dlib::pi);
}
...
//==== Ensure pDat, PMisInd,  pQHPts are ready
double mod1_MNARd2_MLogLikGH(const dbcolvec& theta)
{
...
// int printedBivExp=0;
    ...
    for ( i=0; i<Sz; ++i)
    {
        ...
        { xi(0)=(*pDat)(i,0);  xi(1)=(*pDat)(i,1);
        loglikei=-std::log(detL)-0.5*( trans(xi-mu)*trans(Linv)*Linv*(xi-mu) )(0,0)\
        -std::log( 1+std::exp( alpha01 + alpha11*xi(0) ) )\
        -std::log( 1+std::exp( alpha02 + alpha12*xi(1)   ) );
        }
        ...
        meanCond=mu(0)+rou*(sig1/sig2)*( (*pDat)(i,1)-mu(1) );
        SigCond= sig1*sig1*(1-rou*rou) ;
LogExpectedProbMisXij_mod1(meanCond,SigCond,alpha01, alpha11,logExpectMissProbUni);

        loglikei=-std::log(sig2)-0.5* ( (*pDat)(i,1)-mu(1) )\
         * ( (*pDat)(i,1)-mu(1) )/(sig2*sig2) \
        +logExpectMissProbUni  -std::log( 1+std::exp( alpha02 + alpha12*xi(1) ) );
        ...
        meanCond=mu(1)+rou*(sig2/sig1)*( (*pDat)(i,0)-mu(0) );
        SigCond= sig2*sig2*(1-rou*rou) ;
LogExpectedProbMisXij_mod1(meanCond,SigCond,alpha02, alpha12,logExpectMissProbUni);
//std::cout<<"ExpectMissProbUni due to mis xi2 at row "<<i<<" :"<<std::exp(logExpectMissProbUni)<<"\n";
// std::cout<<trans(Condi_musigbetav)<<"\n";
        loglikei=-std::log(sig1)-0.5* ( (*pDat)(i,0)-mu(0) )\
         * ( (*pDat)(i,0)-mu(0) )/(sig1*sig1) \
        +logExpectMissProbUni  -std::log( 1+std::exp( alpha01 + alpha11*xi(0) ) );
        }
...
dbcolvec mod1_MNARd2_MLogLikGH_Grad(const dbcolvec& theta)
{
  return  gradcdif(mod1_MNARd2_MLogLikGH,theta);
}

dbmat mod1_MNARd2_MLogLikGH_Hess(const dbcolvec& theta)
{
  return  HESScdif(mod1_MNARd2_MLogLikGH,theta);
  	}



class mod1_MNARd2_MLogLikGH_class
{
public:
    typedef ::dbcolvec column_vector;
    typedef matrix<double> general_matrix;
    double operator() (
        const column_vector& x
    ) const { return mod1_MNARd2_MLogLikGH(x); }
    ...
    {
        der = gradcdif(mod1_MNARd2_MLogLikGH,x);
        hess = HESScdif(mod1_MNARd2_MLogLikGH,x);
    }
};
...
//####################
// model 2:
// logit{ prob xi1 mis }= alpha01 + alpha11*xi1;
// logit{ prob xi2 mis }= alpha02 + alpha12*xi2 + alpha22 * ri1;
//####################
...
const double& alpha11,double& LogExpectedProbMisXi1)
{
    ...
    - alpha11* ( std::sqrt(2) *sig* (*pQHPts)(s,0) +mu  )  ) );
    }
...
const double& alpha12, double& LogExpectedProbMisXi2)
{
    ...
    - alpha12 * ( std::sqrt(2) *sig* (*pQHPts)(s,0) +mu  )   ) );
    }
   ...
   for(int j1=0;j1<mQHpts;++j1)
    {
        ...
        qj=mu+L*zj;
        out=out + (wj1*wj2)/ (  ( 1+std::exp( -alpha01-alpha11*qj(0) )  ) \
        *( 1+std::exp( -alpha02 -alpha12*qj(1)  - alpha22  ) )   );
        }
    }
    out=std::log(out) ; // - std::log(dlib::pi);
}
...
LogExpectedProbMisXi1_mod2(meanCond,SigCond,alpha01, alpha11,logExpectMissProbUni);

        loglikei=-std::log(sig2)-0.5* ( (*pDat)(i,1)-mu(1) )\
         * ( (*pDat)(i,1)-mu(1) )/(sig2*sig2) \
        +logExpectMissProbUni  -std::log( 1+std::exp( alpha02 + alpha12*xi(1) + alpha22 ) );
...
LogExpectedProbMisXi2_mod2(meanCond,SigCond,alpha02, alpha12,logExpectMissProbUni);
//std::cout<<"ExpectMissProbUni due to mis xi2 at row "<<i<<" :"<<std::exp(logExpectMissProbUni)<<"\n";
// std::cout<<trans(Condi_musigbetav)<<"\n";
        loglikei=-std::log(sig1)-0.5* ( (*pDat)(i,0)-mu(0) )\
         * ( (*pDat)(i,0)-mu(0) )/(sig1*sig1) \
...
public:
    ...
    ) const
    {
      ...
      // Generate missing data according to logit{ prob xi1 mis }= alpha01 + alpha11*xi1;  logit{ prob xi2 mis }= alpha02 + alpha12*xi2;
  void  ZinExMod_GenerateMisInd(dlib::rand& rndMisInd,const dbmat& Dat,const dbcolvec& alphaVec,dbmat& MisInd)
  {
    int Sz = Dat.nr();  double misProb,RandUniform;
    for(int i = 0; i < Sz; ++i)
    {
    misProb = 1/(1 + std::exp( -alphaVec(0) - alphaVec(1)*Dat(i,0) ) );
    // xi1
    RandUniform = rndMisInd.get_random_double();
    if( RandUniform < misProb)  MisInd(i,0) = 1;
    else  MisInd(i,0) = 0;

    misProb = 1/(1 + std::exp( -alphaVec(2) - alphaVec(3)*Dat(i,1) ) );
    // xi2
    RandUniform = rndMisInd.get_random_double();
